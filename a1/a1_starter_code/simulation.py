    ...
    def run(self, initial_events: list[Event]) -> None:
        ...
        while not self._events.is_empty():
            ...
            if isinstance(event, CheckoutCompleted):
                arrive = event.customer.arrival_time
                check = event.customer.checkout_time
                cust[event.customer] = check - arrive
            for e in new:
                self._events.add(e)
        self.stats['num_customers'] = len(cust)
        self.stats['total_time'] = total
        for name in cust:
            if cust[name] > maxim:
                maxim = cust[name]
        self.stats['max_wait'] = maxim
