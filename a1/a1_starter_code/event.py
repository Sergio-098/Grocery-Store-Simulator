...
class CustomerArrival(Event):
    ...
    def do(self, store: GroceryStore) -> list[Event]:
        ...
        try:
            x = store.enter_line(self.customer)
            if len(store.lines[x]) == 1:
                a = CheckoutStarted(self.timestamp, x)
                lst_event.append(a)
        except NoAvailableLineError:
            a = CustomerArrival(self.timestamp + 1, self.customer)
            lst_event.append(a)
        return lst_event
...
class CheckoutStarted(Event):
    ...
    def do(self, store: GroceryStore) -> list[Event]:
        ...
        lst_event = []
        x = self.line_number
        t = store.next_checkout_time(x)
        a = CheckoutCompleted(self.timestamp + t, x, store.first_in_line(x))
        lst_event.append(a)
        return lst_event
...
class CheckoutCompleted(Event):
    ...
    def do(self, store: GroceryStore) -> list[Event]:
        ...
        lst_event = []
        x = self.line_number
        store.remove_front_customer(x)
        self.customer.checkout_time = self.timestamp
        if store.first_in_line(x) is not None:
            a = CheckoutStarted(self.timestamp, x)
            lst_event.append(a)
        return lst_event
...
class CloseLine(Event):
    ...
    def do(self, store: GroceryStore) -> list[Event]:
        ...
        i = 0
        x = self.line_number
        lst_people = store.close_line(x)
        for p in lst_people:
            temp_lst.insert(0, p)
        for _ in temp_lst:
            c = temp_lst.pop(0)
            t = self.timestamp + i
            a = CustomerArrival(t, c)
            lst_event.append(a)
            i += 1
        return lst_event
...
def create_event_list(event_file: TextIO) -> list[Event]:
    ...
    for instance in events:
        instance.rstrip()
        event = instance.split()
        if 'Arrive' in event:
            lst_items = []
            for i in range(3, len(event), 2):
                x = Item(event[i], int(event[i + 1]))
                lst_items.append(x)
            c = Customer(event[2], lst_items)
            arrive = CustomerArrival(int(event[0]), c)
            lst_events.append(arrive)
        if 'Close' in event:
            close = CloseLine(int(event[0]), int(event[2]))
            lst_events.append(close)
    return lst_events
